// texture.cpp for module09 – same implementation as module07
#include <stb_image.h>
#include "texture.h"
#include <iostream>
...
Texture2D Texture2D::from_file(const char* path, bool flip_y, bool srgb) {
    ...
    GLenum internal_fmt = GL_RGB8, fmt = GL_RGB;
    if      (tex.channels == 1) { internal_fmt = GL_R8;    fmt = GL_RED; }
    else if (tex.channels == 2) { internal_fmt = GL_RG8;   fmt = GL_RG;  }
    else if (tex.channels == 3) { internal_fmt = srgb ? GL_SRGB8        : GL_RGB8;  fmt = GL_RGB;  }
    else if (tex.channels == 4) { internal_fmt = srgb ? GL_SRGB8_ALPHA8 : GL_RGBA8; fmt = GL_RGBA; }

    glGenTextures(1, &tex.id);
    glBindTexture(GL_TEXTURE_2D, tex.id);
    glTexImage2D(GL_TEXTURE_2D, 0, internal_fmt,
                 tex.width, tex.height, 0, fmt, GL_UNSIGNED_BYTE, data);
...
Texture2D Texture2D::create_empty(int w, int h,
                                   GLenum internal_fmt, GLenum fmt, GLenum type) {
    Texture2D tex; tex.width=w; tex.height=h;
    glGenTextures(1, &tex.id);
    glBindTexture(GL_TEXTURE_2D, tex.id);
